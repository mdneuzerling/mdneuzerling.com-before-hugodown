...
These are my notes for my third and final tutorial of useR2018, and the tutorial
I've been looking forward to the most. I *struggle* with missing value 
imputation. It's one of those things which I kind of get the theory of, but
fall over when trying to *do*. So I'm looking forward to
...
```{r load_ozone, warning = FALSE, cache = TRUE}
...
```{r missing_naniar_cluster, cache = TRUE}
...
multiple of $(6, 4)^T$. In other words, the square matrix make the eigenvector 
...
```{r mtcars_prcomp, cache = TRUE}
PC <- prcomp(~ wt + mpg, data = mtcars_scaled)
PC
```

```{r mtcars_pc, cache = TRUE}
PC1 <- PC$rotation[,1]
PC2 <- PC$rotation[,2]
...
```{r mtcars_PC_summary, cache = TRUE}
summary(PC)
```
...
```{r ozone_PCA_dim, results = FALSE, cache = TRUE}
nb <- estim_ncpPCA(ozone, method.cv = "Kfold")
```

```{r ozone_PCA_plot, cache = TRUE}
