...
We need to prepare a new recipe for every `fold_train`. Again, we'll call this `fold_recipe` to indicate that its dependent on the fold.
...
folds_with_xgb <- folds %>% mutate(
    ...
    preds = pmap(list(model = xgb, validate = fold_test), 
                 function(model, validate) predict(model, validate))
)
